...
graph TD
    ...
    %% Entidades Externas / APIs de Soporte
    API_Mod([API Externa de Moderación: Cloudinary / IA])
    Admin_Sys([Panel de Moderación Humana / Admin])

    %% 0. CONTROL DE AUTENTICACIÓN JWT (Protección contra Postman)
    P2_0 --> C_Token{¿El JWT es Válido,<br>Firmado y Vigente?}
    ...
    P2_1 --> C_Plan{¿Supera Límite de<br>Eventos Activos?}
    ...
    P2_2 --> C_FechaInicio{¿Fecha Inicio<br>es Posterior a Hoy?}
    C_FechaInicio -->|No| Err_Fechas([Mensaje: Fechas u Horarios Inválidos])
    C_FechaInicio -->|Sí| C_FechaFin{¿Fecha Fin es Posterior<br>o Igual a Inicio?}
    
    C_FechaFin -->|No| Err_Fechas
    C_FechaFin -->|Sí| P2_3[2.3: Validar Campos Obligatorios y Formatos]

    P2_3 --> C_Campos{¿Campos Obligatorios<br>Están Completos?}
    C_Campos -->|No| Err_Campos([Mensaje: Verifique los Campos Obligatorios])
    C_Campos -->|Sí| C_Cupo{¿El Cupo Máximo<br>es Mayor a Cero?}
    ...
    P2_3A --> C_Sensible{¿La API detectó<br>Contenido Sensible?}
    
    %% CAMINO NEGATIVO: Se persiste el rechazo automático sin crear el evento activo
    ...
    %% CAMINO POSITIVO: Se guarda todo de forma exitosa
    C_Sensible -->|No| P2_4[2.4: Procesar y Persistir Evento Limpio]

    %% 4. PERSISTENCIA EN CASCADA (Solo camino limpio)
    P2_4 -->|1. Guardar Registro Base Estado PENDIENTE| D10_Ev
    ...
    %% 5. DISPARADOR DE ALERTA ASINCRÓNICA PARA EL STAFF (Solo para los Pendientes)
    P2_4 -->|id_evento confirmado como PENDIENTE| P2_5[2.5: Notificar Moderación Humana]
    P2_5 -->|Alerta de Nuevo Evento para Revisión Visual| Admin_Sys
    
    %% Salidad de Cierre del Proceso Exitoso
    P2_4 -->|Response 200 OK| Success_Ev([Evento Creado: En Proceso de Evaluación])
